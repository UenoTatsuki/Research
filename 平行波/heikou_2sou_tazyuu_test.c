#include <stdio.h>
#include <math.h>
#include <stdlib.h>
#include <time.h>

#define NUMBER_OF_JUNCTION 20
#define NUMBER_OF_SINDOUSI_TATE 50
#define NUMBER_OF_SINDOUSI_YOKO 50
#define START_OF_TRIGER 2500
#define END_OF_TRIGER 2510
#define NUM_PULSE 1

int main(){
    double Cj = NUMBER_OF_JUNCTION*10.0; //[aF]
    double C = 2.0;//[aF]
    double R = 10.0; //[G]
    double Rj = 0.001;//[G]
    double Vd8[NUMBER_OF_SINDOUSI_TATE+1][NUMBER_OF_SINDOUSI_YOKO+1] = {0.0}; //[V]
    double Vd4[NUMBER_OF_SINDOUSI_TATE+1][NUMBER_OF_SINDOUSI_YOKO+1] = {0.0}; //[V]
    double Vin = 0.01;//[V]
    double Vn8[NUMBER_OF_SINDOUSI_TATE+2][NUMBER_OF_SINDOUSI_YOKO+2] = {0.0};  //[V]
    double Vn4[NUMBER_OF_SINDOUSI_TATE+2][NUMBER_OF_SINDOUSI_YOKO+2] = {0.0};  //[V]
    double Q8[NUMBER_OF_SINDOUSI_TATE+1][NUMBER_OF_SINDOUSI_YOKO+1] = {0.0}; 
    double Q4[NUMBER_OF_SINDOUSI_TATE+1][NUMBER_OF_SINDOUSI_YOKO+1] = {0.0}; 
    double t = 0.0; //[ns]
    double dt = 0.1, dq;
    double e = 0.1602,r;
    int L8[NUMBER_OF_SINDOUSI_TATE+1][NUMBER_OF_SINDOUSI_YOKO+1] = {0};
    int L4[NUMBER_OF_SINDOUSI_TATE+1][NUMBER_OF_SINDOUSI_YOKO+1] = {0};
    double q_sum8[NUMBER_OF_SINDOUSI_TATE+1][NUMBER_OF_SINDOUSI_YOKO+1] = {0.0};
    double q_sum4[NUMBER_OF_SINDOUSI_TATE+1][NUMBER_OF_SINDOUSI_YOKO+1] = {0.0};
    double dEup8[NUMBER_OF_SINDOUSI_TATE+1][NUMBER_OF_SINDOUSI_YOKO+1] = {0.0};
    double dEup4[NUMBER_OF_SINDOUSI_TATE+1][NUMBER_OF_SINDOUSI_YOKO+1] = {0.0};
    double dEdown8[NUMBER_OF_SINDOUSI_TATE+1][NUMBER_OF_SINDOUSI_YOKO+1] = {0.0};
    double dEdown4[NUMBER_OF_SINDOUSI_TATE+1][NUMBER_OF_SINDOUSI_YOKO+1] = {0.0};
    int up = 0;
    int down = 0;
    int w = 0;
    double wt = 0;
    int i, j = 0;
    double wt_min = 1000.0;
    int A, B = 0;
    int a, b = 0;
    int pulse1[NUM_PULSE][2] = {{0,21}};
    int pulse2[NUM_PULSE][2] = {{0,29}};
    int pulse3[NUM_PULSE][2] = {{0,25}};
    int x, y = 0;
    int m = 0;
    int printkaisu = 0;
    //8結合層の周囲の振動子
    double Vl8[NUMBER_OF_SINDOUSI_TATE+1][NUMBER_OF_SINDOUSI_YOKO+1] = {0.0};
    double Vr8[NUMBER_OF_SINDOUSI_TATE+1][NUMBER_OF_SINDOUSI_YOKO+1] = {0.0};
    double Vu8[NUMBER_OF_SINDOUSI_TATE+1][NUMBER_OF_SINDOUSI_YOKO+1] = {0.0};
    double Vb8[NUMBER_OF_SINDOUSI_TATE+1][NUMBER_OF_SINDOUSI_YOKO+1] = {0.0};
    double V18[NUMBER_OF_SINDOUSI_TATE+1][NUMBER_OF_SINDOUSI_YOKO+1] = {0.0};
    double V28[NUMBER_OF_SINDOUSI_TATE+1][NUMBER_OF_SINDOUSI_YOKO+1] = {0.0};
    double V38[NUMBER_OF_SINDOUSI_TATE+1][NUMBER_OF_SINDOUSI_YOKO+1] = {0.0};
    double V48[NUMBER_OF_SINDOUSI_TATE+1][NUMBER_OF_SINDOUSI_YOKO+1] = {0.0};
    double Vs8[NUMBER_OF_SINDOUSI_TATE+1][NUMBER_OF_SINDOUSI_YOKO+1] = {0.0};
    //4結合層の周囲の振動子
    double Vl4[NUMBER_OF_SINDOUSI_TATE+1][NUMBER_OF_SINDOUSI_YOKO+1] = {0.0};
    double Vr4[NUMBER_OF_SINDOUSI_TATE+1][NUMBER_OF_SINDOUSI_YOKO+1] = {0.0};
    double Vu4[NUMBER_OF_SINDOUSI_TATE+1][NUMBER_OF_SINDOUSI_YOKO+1] = {0.0};
    double Vb4[NUMBER_OF_SINDOUSI_TATE+1][NUMBER_OF_SINDOUSI_YOKO+1] = {0.0};
    double Vs4[NUMBER_OF_SINDOUSI_TATE+1][NUMBER_OF_SINDOUSI_YOKO+1] = {0.0};


    srand((unsigned)time(NULL));

    //8結合層でバイアス電圧をかける
    for(b = 1; b < NUMBER_OF_SINDOUSI_TATE+1; b++){
        for(a = 1; a < NUMBER_OF_SINDOUSI_YOKO+1; a++){
            if((a+b)%2 == 0){
                Vd8[b][a] = 0.0074;
            }else{
                Vd8[b][a] = -0.0074;
            }
        }
    }

    //４結合層でバイアス電圧をかける
    for(b = 1; b < NUMBER_OF_SINDOUSI_TATE+1; b++){
        for(a = 1; a < NUMBER_OF_SINDOUSI_YOKO+1; a++){
            if((a+b)%2 == 0){
                Vd4[b][a] = 0.0074;
            }else{
                Vd4[b][a] = -0.0074;
            }
        }
    }

    //ループ開始
	while(t <= START_OF_TRIGER + 500){

        //トリガを印加
        if(t > START_OF_TRIGER && t < END_OF_TRIGER){
            for(m = 0; m < NUM_PULSE; m++){
                if((pulse1[m][0]+pulse1[m][1])%2 == 1){
                    Vn8[pulse1[m][0]][pulse1[m][1]] = Vin;
                }else{
                    Vn8[pulse1[m][0]][pulse1[m][1]] = -Vin;
                }
            }
        }else{
            for(m = 0; m < NUM_PULSE; m++){
                if((pulse1[m][0]+pulse1[m][1])%2 == 1){
                    Vn8[pulse1[m][0]][pulse1[m][1]] = 0;
                }else{
                    Vn8[pulse1[m][0]][pulse1[m][1]] = 0;
                }
            }
        }

        if(t > START_OF_TRIGER && t < END_OF_TRIGER){
            for(m = 0; m < NUM_PULSE; m++){
                if((pulse2[m][0]+pulse2[m][1])%2 == 1){
                    Vn8[pulse2[m][0]][pulse2[m][1]] = Vin;
                }else{
                    Vn8[pulse2[m][0]][pulse2[m][1]] = -Vin;
                }
            }
        }else{
            for(m = 0; m < NUM_PULSE; m++){
                if((pulse2[m][0]+pulse2[m][1])%2 == 1){
                    Vn8[pulse2[m][0]][pulse2[m][1]] = 0;
                }else{
                    Vn8[pulse2[m][0]][pulse2[m][1]] = 0;
                }
            }
        }

        if(t > START_OF_TRIGER + 30 && t < END_OF_TRIGER + 30){
            for(m = 0; m < NUM_PULSE; m++){
                if((pulse3[m][0]+pulse3[m][1])%2 == 1){
                    Vn4[pulse3[m][0]][pulse3[m][1]] = Vin;
                }else{
                    Vn4[pulse3[m][0]][pulse3[m][1]] = -Vin;
                }
            }
        }else{
            for(m = 0; m < NUM_PULSE; m++){
                if((pulse3[m][0]+pulse3[m][1])%2 == 1){
                    Vn4[pulse3[m][0]][pulse3[m][1]] = 0;
                }else{
                    Vn4[pulse3[m][0]][pulse3[m][1]] = 0;
                }
            }
        }

        //周囲の振動子と接続
        for(b = 1; b < NUMBER_OF_SINDOUSI_TATE+1; b++){
            for(a = 1; a < NUMBER_OF_SINDOUSI_YOKO+1; a++){
                Vl8[b][a] = Vn8[b][a-1];
                Vr8[b][a] = Vn8[b][a+1];
                Vu8[b][a] = Vn8[b+1][a];
                Vb8[b][a] = Vn8[b-1][a];
                V18[b][a] = Vn8[b-1][a-1];
                V28[b][a] = Vn8[b-1][a+1];
                V38[b][a] = Vn8[b+1][a-1];
                V48[b][a] = Vn8[b+1][a+1];
                Vs8[b][a] = Vn4[b][a];
                Vl4[b][a] = Vn4[b][a-1];
                Vr4[b][a] = Vn4[b][a+1];
                Vu4[b][a] = Vn4[b+1][a];
                Vb4[b][a] = Vn4[b-1][a];
                Vs4[b][a] = Vn8[b][a];
            }
        } 
                
        //電荷計算
        for(b = 1; b < NUMBER_OF_SINDOUSI_TATE+1; b++){
            for(a = 1; a < NUMBER_OF_SINDOUSI_YOKO+1; a++){
                q_sum8[b][a] = (-NUMBER_OF_JUNCTION*(-Cj*Q8[b][a]-C*Cj*(Vl8[b][a]+Vr8[b][a]+Vu8[b][a]+Vb8[b][a]+V18[b][a]+V28[b][a]+V38[b][a]+V48[b][a]+Vs8[b][a]))-Cj*L8[b][a]*e)/(9*NUMBER_OF_JUNCTION*C+Cj);
            }
        }

        for(b = 1; b < NUMBER_OF_SINDOUSI_TATE+1; b++){
            for(a = 1; a < NUMBER_OF_SINDOUSI_YOKO+1; a++){
                Vn8[b][a] = q_sum8[b][a]/Cj;
            }
        }

        for(b = 1; b < NUMBER_OF_SINDOUSI_TATE+1; b++){
            for(a = 1; a < NUMBER_OF_SINDOUSI_YOKO+1; a++){
                q_sum4[b][a] = (-NUMBER_OF_JUNCTION*(-Cj*Q4[b][a]-C*Cj*(Vl4[b][a]+Vr4[b][a]+Vu4[b][a]+Vb4[b][a]+Vs4[b][a]))-Cj*L4[b][a]*e)/(5*NUMBER_OF_JUNCTION*C+Cj);
            }
        }

        for(b = 1; b < NUMBER_OF_SINDOUSI_TATE+1; b++){
            for(a = 1; a < NUMBER_OF_SINDOUSI_YOKO+1; a++){
                Vn4[b][a] = q_sum4[b][a]/Cj;
            }
        }

        /*Vnの出力*/
        printf("%f %f %f %f %f %f %f %f %f %f %f %f %f %f\n",t, Vn4[1][25], Vn4[2][25], Vn4[48][25], Vn4[49][25]);
        
        //dEを計算
        for(b = 1; b < NUMBER_OF_SINDOUSI_TATE+1; b++){
            for(a = 1; a < NUMBER_OF_SINDOUSI_YOKO+1; a++){
                dEup8[b][a] = e*((-(NUMBER_OF_JUNCTION-1)*9+2*9*L8[b][a])*C*e+Cj*(2*Q8[b][a]-e)+2*C*Cj*(Vl8[b][a]+Vr8[b][a]+Vu8[b][a]+Vb8[b][a]+V18[b][a]+V28[b][a]+V38[b][a]+V48[b][a]+Vs8[b][a]))/(2*Cj*(9*NUMBER_OF_JUNCTION*C+Cj));
                dEdown8[b][a] = -e*(-(-(NUMBER_OF_JUNCTION-1)*9-2*9*L8[b][a])*C*e+Cj*(2*Q8[b][a]+e)+2*C*Cj*(Vl8[b][a]+Vr8[b][a]+Vu8[b][a]+Vb8[b][a]+V18[b][a]+V28[b][a]+V38[b][a]+V48[b][a]+Vs8[b][a]))/(2*Cj*(9*NUMBER_OF_JUNCTION*C+Cj));

                dEup4[b][a] = e*((-(NUMBER_OF_JUNCTION-1)*5+2*5*L4[b][a])*C*e+Cj*(2*Q4[b][a]-e)+2*C*Cj*(Vl4[b][a]+Vr4[b][a]+Vu4[b][a]+Vb4[b][a]+Vs4[b][a]))/(2*Cj*(5*NUMBER_OF_JUNCTION*C+Cj));
                dEdown4[b][a] = -e*(-(-(NUMBER_OF_JUNCTION-1)*5-2*5*L4[b][a])*C*e+Cj*(2*Q4[b][a]+e)+2*C*Cj*(Vl4[b][a]+Vr4[b][a]+Vu4[b][a]+Vb4[b][a]+Vs4[b][a]))/(2*Cj*(5*NUMBER_OF_JUNCTION*C+Cj));

                if(dEup8[b][a] > 0){
                    for(j = 0; j < NUMBER_OF_JUNCTION-L8[b][a]; j++){
                        r = (double)rand()/RAND_MAX;
                        wt = (e*e*Rj/dEup8[b][a])*log(1/r);
                        
                        if(wt < dt && wt < wt_min){
                            up = 1;
                            down = 0;
                            wt_min = wt;
                            A = a;
                            B = b;
                            w = 8;
                        }
                    }
                }
                if(dEdown8[b][a] > 0){
                    for(j = 0; j < NUMBER_OF_JUNCTION+L8[b][a]; j++){
                        r=(double)rand()/RAND_MAX;
                        wt = (e*e*Rj/dEdown8[b][a])*log(1/r);

                        if(wt < dt && wt < wt_min){
                            up = 0;
                            down = 1;
                            wt_min = wt;
                            A = a;
                            B = b;
                            w = 8;
                        }
                    }
                }

                if(dEup4[b][a] > 0){
                    for(j = 0; j < NUMBER_OF_JUNCTION-L4[b][a]; j++){
                        r = (double)rand()/RAND_MAX;
                        wt = (e*e*Rj/dEup4[b][a])*log(1/r);
                        
                        if(wt < dt && wt < wt_min){
                            up = 1;
                            down = 0;
                            wt_min = wt;
                            A = a;
                            B = b;
                            w = 4;
                        }
                    }
                }
                if(dEdown4[b][a] > 0){
                    for(j = 0; j < NUMBER_OF_JUNCTION+L4[b][a]; j++){
                        r=(double)rand()/RAND_MAX;
                        wt = (e*e*Rj/dEdown4[b][a])*log(1/r);

                        if(wt < dt && wt < wt_min){
                            up = 0;
                            down = 1;
                            wt_min = wt;
                            A = a;
                            B = b;
                            w = 4;
                        }
                    }
                }
            }
        }

        //トンネルする振動子を決定　& 電荷チャージ
        if(up == 1 && w == 8){
            for(b = 1; b < NUMBER_OF_SINDOUSI_TATE+1; b++){
                for(a = 1; a < NUMBER_OF_SINDOUSI_YOKO+1; a++){
                    Q8[b][a] += (Vd8[b][a]-Vn8[b][a])*wt_min/R;
                    Q4[b][a] += (Vd4[b][a]-Vn4[b][a])*wt_min/R;
                }
            }
            L8[B][A] += 1;

            if(L8[B][A] == NUMBER_OF_JUNCTION){
                L8[B][A] = 0;
                Q8[B][A] -= e;
            }
            t += wt_min;
            up = 0;


        }else if(down == 1 && w == 8){
            for(b = 1; b < NUMBER_OF_SINDOUSI_TATE+1; b++){
                for(a = 1; a < NUMBER_OF_SINDOUSI_YOKO+1; a++){
                    Q8[b][a] += (Vd8[b][a]-Vn8[b][a])*wt_min/R;
                    Q4[b][a] += (Vd4[b][a]-Vn4[b][a])*wt_min/R;
                }
            }
            L8[B][A] -= 1;
            if(L8[B][A] == -NUMBER_OF_JUNCTION){
                L8[B][A] = 0;
                Q8[B][A] += e;
            }
            t += wt_min;
            down = 0;


        }else if(up == 1 && w == 4){
            for(b = 1; b < NUMBER_OF_SINDOUSI_TATE+1; b++){
                for(a = 1; a < NUMBER_OF_SINDOUSI_YOKO+1; a++){
                    Q8[b][a] += (Vd8[b][a]-Vn8[b][a])*wt_min/R;
                    Q4[b][a] += (Vd4[b][a]-Vn4[b][a])*wt_min/R;
                }
            }
            L4[B][A] += 1;
            if(L4[B][A] == NUMBER_OF_JUNCTION){
                L4[B][A] = 0;
                Q4[B][A] -= e;
            }
            t += wt_min;
            up = 0;


        }else if(down == 1 && w == 4){
            for(b = 1; b < NUMBER_OF_SINDOUSI_TATE+1; b++){
                for(a = 1; a < NUMBER_OF_SINDOUSI_YOKO+1; a++){
                    Q8[b][a] += (Vd8[b][a]-Vn8[b][a])*wt_min/R;
                    Q4[b][a] += (Vd4[b][a]-Vn4[b][a])*wt_min/R;
                }
            }
            L4[B][A] -= 1;
            if(L4[B][A] == -NUMBER_OF_JUNCTION){
                L4[B][A] = 0;
                Q4[B][A] += e;
            }
            t += wt_min;
            down = 0;


        }else{
            for(b = 1; b < NUMBER_OF_SINDOUSI_TATE+1; b++){
                for(a = 1; a < NUMBER_OF_SINDOUSI_YOKO+1; a++){
                    Q8[b][a] += (Vd8[b][a]-Vn8[b][a])*dt/R;
                    Q4[b][a] += (Vd4[b][a]-Vn4[b][a])*dt/R;
                }
            }
            t += dt;
        }
        
        up = 0;
        down = 0;
        wt = 1000.0;
        wt_min = 1000.0;
        A = 0;
        B = 0;
	}
	return 0;
}
